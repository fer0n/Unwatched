...
public final class CloudStorageSync: ObservableObject {
    ...
    /// Reads and writes go through the mirror, so a value stays available while iCloud is off.
    /// `ubiquitousKvs` is kept for `synchronize()`, which has no equivalent on the mirror.
    private let store = CloudKeyValueStore.shared
    ...
    private func didChangeExternally(notification: Notification) {
        ...
        // iCloud is reachable and has the last word on these keys, so the mirror follows it here
        // — including for keys it dropped, which is the only place a removal can be recognized.
        store.refreshMirror(forKeys: keys)
...
// Wrap calls to CloudKeyValueStore (NSUbiquitousKeyValueStore plus its local mirror)
extension CloudStorageSync {
    ...
    public func url(for key: String) -> URL? {
        store.string(forKey: key).flatMap(URL.init(string:))
    }
    ...
    public func date(for key: String) -> Date? {
        guard let obj = store.object(forKey: key) else { return nil }
        return obj as? Date
    }
    ...
    public func int(for key: String) -> Int? {
        if store.object(forKey: key) == nil { return nil }
